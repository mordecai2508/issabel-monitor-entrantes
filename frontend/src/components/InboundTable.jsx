...
export default function InboundTable() {
  ...
  const fetchData = useCallback(async (activeFilters, currentPage) => {
    ...
    try {
      ...
      const data = await api.inboundCalls(params.toString());
      setRows(data.data || []);
  ...
  function handleClear() {
    const cleared = { from: today, to: today, trunk: '', origin: '', disposition: '' };
    setFilters(cleared);
    setPage(1);
  ...
  function buildExportUrl(format) {
    ...
    if (appliedFilters.origin)      params.set('origin',      appliedFilters.origin);
    if (appliedFilters.disposition) params.set('disposition', appliedFilters.disposition);
    return `/api/calls/inbound/export?${params.toString()}`;
  }
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-semibold text-slate-100 mb-6">Búsqueda de llamadas entrantes</h1>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 mb-4">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          {/* Origin */}
          <div>
            <label className="block text-xs text-slate-400 mb-1">Número origen</label>
            <input
              type="text"
                ...
                <tbody>
                  {sortedRows.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 whitespace-nowrap text-slate-400">{row.channel}</td>
